...
// Coordenadas das aduanas (as mesmas usadas no aplicativo)
const ADUANA_COORDINATES: Record<string, { lat: number; lng: number }> = {
  // Aduanas brasileiras
 ...
 */
export async function migrateAduanas() {
  try {
    ...
    if (existingAduanas.length > 0) {
      console.log(`Encontradas ${existingAduanas.length} aduanas no banco. Continuando com a migração...`);
    }
